...
void tof_init(){
	...
	// Fourth, write default configuration to ToF sensor (see "tof_config.h")
	...
	// Fifth, enable range data collection (ranging)
		tof_enable(true);

	// Sixth, wait until some data is ready (ensures successful boot)
		while (tof_check_data_ready() == 0) {
    		sleep_ms(2);
		}

	// Seventh, clear read interrupt, stop ranging
		tof_clear_int();
 ...
 * Arguments
		> N/A
 * Returns
        > bool: true = active-high, false = active-low
*/
bool tof_get_int_polarity(){
	...
	// Second, read from interrupt priority ToF register
		tof_read_byte(GPIO_HV_MUX__CTRL, &polarity);

	// Third, interpret polarity from register value
		polarity = polarity & 0b00010000;		// isolate int polarity bit (bit 4)
		active_high = !(polarity >> 4);

	// Lastly, return polarity
		return active_high;
}
