...
int tof_init(){
	// First, initialize the I2C bus used for ToF communication
		init_i2c(TOF_I2C_PORT, TOF_I2C_BAUDRATE, TOF_I2C_SDA, TOF_I2C_SCL);

	// Second, init GPIO pins related to ToF sensor
		//i: GPIO1 pin (input) - UNUSED ON DOORSENSE V3
			//gpio_init(TOF_GPIO1_PIN);
			//gpio_set_dir(TOF_GPIO1_PIN, SET_GPIO_INPUT);

		//ii: XSHUT pin (output) - HARDWIRED TO +1V8 SO THAT TOF IS ALWAYS ACTIVE ON DOORSENSE V3
